				  ...
				  struct pldm_msg *msg)
{
	...
	if (transfer_flag == PLDM_RDE_START_AND_END ||
	    transfer_flag == PLDM_RDE_END) {
		add_checksum = true;
	}
	...
	uint32_t data_only_length =
		data_length_bytes -
		(add_checksum ? sizeof(data_integrity_checksum) : 0);

	rc = pldm_msgbuf_insert_array(buf, data_only_length,
				      (const uint8_t *)data, data_only_length);

	if (rc != PLDM_SUCCESS) {
		fprintf(stderr, "Data Array insersion failed\n");
		return rc;
	}
	if (add_checksum) {
				  ...
				  uint32_t *data_integrity_checksum)
{
	...
	if (msg == NULL || data_transfer_handle == NULL ||
	    operation_id == NULL || transfer_flag == NULL ||
	...
	size_t data_len = *data_length_bytes;

	if (*transfer_flag == PLDM_RDE_END ||
	    *transfer_flag == PLDM_RDE_START_AND_END) {
		add_checksum = true;
		data_len -= sizeof(*data_integrity_checksum);
	}
	rc = pldm_msgbuf_extract_array(buf, data_len, data, data_len);
	if (rc) {
				     ...
				     uint8_t *transfer_operation)
{
	...
	if (payload_length != PLDM_RDE_MULTIPART_RECEIVE_REQ_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
	...
	struct pldm_msg *msg)
{
	...
	rc = pldm_msgbuf_init_errno(
		buf, PLDM_RDE_MULTIPART_RECEIVE_RESP_FIXED_BYTES, msg->payload,
	...
	uint8_t *data, uint32_t *data_integrity_checksum)
{
	...
	if ((msg == NULL) || (completion_code == NULL) ||
	    (transfer_flag == NULL) || (data_transfer_handle == NULL) ||
	    (data_length_bytes == NULL) || (data == NULL)) {
		return PLDM_ERROR_INVALID_DATA;
	}

	if (payload_length < PLDM_RDE_MULTIPART_RECEIVE_RESP_FIXED_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
	rc = pldm_msgbuf_init_errno(buf,
				    PLDM_RDE_MULTIPART_RECEIVE_RESP_FIXED_BYTES,
				      ...
				      rde_op_id *operation_id)
{
	...
	if (payload_length < PLDM_RDE_OPERATION_COMPLETE_REQ_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
				       ...
				       struct pldm_msg *msg)
{
	...
	rc = pldm_msgbuf_init_errno(buf, PLDM_RDE_OPERATION_COMPLETE_RESP_BYTES,
				    msg->payload,
				    PLDM_RDE_OPERATION_COMPLETE_RESP_BYTES);
				       ...
				       uint8_t *completion_code)
{
	...
	if (payload_length < PLDM_RDE_OPERATION_COMPLETE_RESP_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
				    ...
				    struct pldm_msg *msg)
{
	...
	rc = pldm_msgbuf_init_errno(buf, PLDM_RDE_OPERATION_STATUS_REQ_BYTES,
				    msg->payload,
				    PLDM_RDE_OPERATION_STATUS_REQ_BYTES);
	if (rc) {
		return rc;
	}
				    ...
				    rde_op_id *operation_id)
{
	...
	if (payload_length < PLDM_RDE_OPERATION_STATUS_REQ_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
	...
	const char *etag, const uint8_t *response_payload, struct pldm_msg *msg)
{
	...
	rc = pldm_msgbuf_init_errno(buf,
				    PLDM_RDE_OPERATION_STATUS_RESP_FIXED_BYTES,
				    msg->payload,
				    PLDM_RDE_OPERATION_STATUS_RESP_FIXED_BYTES +
	...
	const struct pldm_rde_op_entry *operations, struct pldm_msg *msg)
{
	...
	rc = pldm_msgbuf_init_errno(
		...
		PLDM_RDE_OPERATION_ENUMERATE_RESP_FIXED_BYTES +
			(operation_count * sizeof(struct pldm_rde_op_entry)));
	if (rc) {
		return rc;
	}

	pldm_msgbuf_insert(buf, completion_code);
	pldm_msgbuf_insert(buf, operation_count);

	for (int i = 0; i < operation_count; ++i) {
		pldm_msgbuf_insert(buf, operations[i].resource_id);
		pldm_msgbuf_insert(buf, operations[i].operation_id);
		pldm_msgbuf_insert(buf, operations[i].operation_type);
	}
					...
					struct pldm_rde_op_entry *operations)
{
	...
	for (int i = 0; i < *operation_count; ++i) {
		pldm_msgbuf_extract_p(buf, &operations[i].resource_id);
		pldm_msgbuf_extract_p(buf, &operations[i].operation_id);
		pldm_msgbuf_extract_p(buf, &operations[i].operation_type);
	}
